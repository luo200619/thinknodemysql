...
var MysqlDriver = function (_Driver) {
  ...
  */
   ...
   * @版权     思智捷信息管理系统
   * @版本     1.0.0
   * @param  {[type]}   !this.mysql [description]
   * @return {[type]}               [description]
   */
		if (!that.mysql) {
			that.mysql = mysqldb.getInstance(that.config);
		}
		return _this;
	}
	/**
					...
					while (1) {
						switch (_context4.prev = _context4.next) {
							case 0:
								...
								_context4.prev = 1;
								_context4.next = 4;
								return that.mysql.getPoolHandle();
		...
		value: function _parse_join_table() {
			...
			if (tools.is_string(expression)) {
				var newexpression = expression.replace(/__([A-Z0-9_-]+)__/g, function ($1) {
					return that.config.DB_PREFIX + $1.replace(/__/g, '').toLowerCase();
				});
